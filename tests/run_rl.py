...
def test_vd_rl(environment_name, exp, exp_par, n_epochs=10000, plr=0.001, vdlr=0.001, clr=0.001, grad_clip=1,
    ...
    # Logging directory
    logdir = os.getcwd() + '/VDR_rl/' + environment_name + '/' + exp + f'/{int(run_id)}'
...
def test_mc_rl(environment_name, exp, exp_par, n_epochs=10000, plr=0.001, grad_clip=1, save_log=False, run_id=0):
    print('rl', environment_name)
    tasks = {
        'pocman': (pocman.MicroPOCMAN, pocman.LocalPOCFeatures, pocman.POCLoss, pocman.POCReference),
        'cartpole': (cartpole.CartPoleEnv, cartpole.CartPoleFeatures, cartpole.CartPoleLoss, None),
    ...
    if exp == 'bootstrap':
        ...
        raise ValueError('Invalid exploration method')
    # Logging directory
    logdir = os.getcwd() + '/MC_rl/' + environment_name + '/' + exp + f'/{int(run_id)}'
    pathlib.Path(logdir).mkdir(parents=True)
    if save_log == True:
        writer = SummaryWriter(logdir)
    else:
        writer = None
    learner = MonteCarlo(reference=None, policy=policy, exploration=exploration, p_explore=explore, temperature=temp,
...
def test_reslope(env, plr, clip, exp, exp_param):
    # run on CPU
    gpu_id = None
    # if len(sys.argv) == 1:
    util.reseed(90210, gpu_id=gpu_id)
    ...
    for i in range(10):
        util.reseed(seeds[i], gpu_id=gpu_id)
        test_reslope_rl(environment_name=env, n_epochs=10000, plr=plr, grad_clip=clip,exp=exp, exp_par=exp_param,
                   run_id=i+1, save_log=False)

def test_mcarlo(env, plr, clip, exp, exp_param):
    # run on CPU
    gpu_id = None
    # if len(sys.argv) == 1:
    util.reseed(90210, gpu_id=gpu_id)
