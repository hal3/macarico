...
#    @profile
    ...
    def stochastic_with_probability(self, state, temperature=1):
        ...
        if len(state.actions) != self.n_actions:
            disallow = torch.zeros(1, self.n_actions)
...
#    @profile
    def greedy(self, state, pred_costs=None, deviate_to=None):
        ...
        if len(state.actions) == self.n_actions:
            return pred_costs.min(1)[1].numpy()[0]
        best = None
...
#    @profile
    def forward_partial_complete(self, pred_costs, truth, actions):
        ...
        if isinstance(truth, list):
            truth0 = truth
            truth = torch.ones(1, self.n_actions)
            for k in truth0:
                truth[0,k] = 0.
        ...
        if len(actions) == self.n_actions:
            ...
            obj = 0.
            for a in actions:
                obj += self.distance(pred_costs[0,a], Var(truth[0,a], requires_grad=False))
            return obj
