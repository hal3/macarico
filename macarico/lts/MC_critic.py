...
class MonteCarloC(macarico.Learner):
    def __init__(self, policy, critic, actor, reference=None, p_rollin_ref=NoAnnealing(0), p_rollout_ref=NoAnnealing(0.5),
    ...
    def forward(self, state):
        ...
        critic_costs = self.critic(self.actor(state).data)
        if not self.explore():
            a = a_pol
        else:
            dev_a, iw, probs = self.do_exploration(a_costs, state.actions)
            ...
            self.critic_costs.append(critic_costs)
            self.dev_a_probs.append(probs)
        self.t += 1
        return a
    ...
    def do_exploration(self, costs, dev_actions):
        ...
        if self.exploration == BanditLOLS.EXPLORE_UNIFORM:
            return int(np.random.choice(list(dev_actions))), len(dev_actions), \
                   np.ones(self.policy.n_actions, 1)/self.policy.n_actions
    ...
    def get_objective(self, loss, final_state=None):
        ...
        if self.dev_a is not None:
            for dev_t, dev_a, dev_actions, dev_costs, critic_costs, dev_imp_weight, dev_a_probs in \
                    zip(self.dev_t, self.dev_a, self.dev_actions, self.dev_costs, self.critic_costs,
                        self.dev_imp_weight, self.dev_a_probs):
                dev_costs_data = dev_costs.data if isinstance(dev_costs, Var) else \
                    dev_costs.data() if isinstance(dev_costs, macarico.policies.bootstrap.BootstrapCost) else \
                        None
                costs = critic_costs.data.numpy()[0]
                feedback = critic_costs.data.numpy()[0,dev_a] - np.dot(dev_a_probs, costs)
                self.build_truth_vector(feedback, dev_a, dev_imp_weight, dev_costs_data)
                loss_var = self.critic.update(self.critic_costs[dev_t][0,dev_a],
                                              torch.Tensor([final_state.loss_to_go(dev_t)]))
                critic_loss += loss_var.data.numpy()
                critic_loss_var += loss_var
                ...
                policy_loss += loss_var.data.numpy()
                pol_squared_loss += (final_state.loss_to_go(dev_t) - (dev_costs_data.numpy())[dev_a])**2
        ...
        critic_loss /= self.t
        policy_loss /=self.t
        pol_squared_loss /= self.t

        self.t, self.dev_t, self.dev_a, self.dev_actions, self.dev_imp_weight, self.dev_costs, self.critic_costs, \
        self.rollout = [None] * 8
        return total_loss_var + critic_loss_var, [critic_loss, policy_loss, pol_squared_loss]
